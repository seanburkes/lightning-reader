...
pub fn load_state(book: &BookId) -> Option<AppStateRecord> {
    ...
    records.into_iter().find(|r| r.book.id == book.id || r.book.path == book.path)
}

pub fn save_state(record: &AppStateRecord) -> std::io::Result<()> {
    let dir = config_dir().ok_or_else(|| std::io::Error::new(std::io::ErrorKind::NotFound, "no config dir"))?;
    ...
    let mut records: Vec<AppStateRecord> = fs::read(&path)
        ...
        .unwrap_or_default();
    if let Some(existing) = records.iter_mut().find(|r| r.book.id == record.book.id || r.book.path == record.book.path) {
